...
def test_send_uses_stored_session_project_dir_when_request_omits_it(monkeypatch, tmp_path):
    ...
    async def fake_stream(
        ...
        *,
        ...
        captured["disable_design_system"] = bool(disable_design_system)
...
def test_send_forwards_disable_design_system_to_runner(monkeypatch):
    monkeypatch.delenv("API_AUTH_TOKEN", raising=False)
    monkeypatch.setattr(chat_api, "_infer_default_project_dir", lambda: None)
    ...
    response = client.post(
        "/api/chat/send",
        json={
            "session_id": "session-disable-design-system-1",
            "message": "build direct html",
